...
/*   Updated: 2022/11/27 11:16:26 by tgiraudo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	map->x1 = map->start_x + ((map->offset_x * xx1) - (map->offset_y * yy1)) / sqrt(2);
	map->y1 = map->start_y - (map->tab[yy1][xx1] * 0.8) + ((map->offset_x * xx1) + (map->offset_y * yy1)) / sqrt(6);
	map->x2 = map->start_x + ((map->offset_x * xx2) - (map->offset_y * yy2)) / sqrt(2);
	map->y2 = map->start_y - (map->tab[yy2][xx2] * 0.8) + ((map->offset_x * xx2) + (map->offset_y * yy2)) / sqrt(6);
	if (map->tab[yy2][xx2] <= 0)
		map->color = 0x000CFF;
	else if (map->tab[yy2][xx2] <= 10)
		map->color = 0x0C961F;
	else if (map->tab[yy2][xx2] <= 60)
		map->color = 0xB0855A;
	else if (map->tab[yy2][xx2] <= 100)
		map->color = 0x623509;
	else 
		map->color = 0xFFFFFF;
}
...
void	ft_draw_map(t_map *map)
{
	...
	i = 0;
	mlx_string_put(map->mlx, map->win, 10, 10, 0xFFFFFF, "Controle : ");
	...
	mlx_string_put(map->mlx, map->win, 1210, 775, 0xFFFFFF, "tgiraudo");
	while(map->depth > i)
	{
		j = 0;
		while(map->length > j)
